...
设置页发现新版本后点「下载并安装」即可，**不再弹二次确认**（点按钮本身就是确认）：

1. 下载官方安装程序 `ForzaGallerySync-<版本>-setup.exe` 并校验 SHA256；
2. 退出本程序，由独立脚本静默运行安装程序（`/SILENT`，会显示安装进度窗口）；
3. 装完自动启动新版本；安装失败或被取消则自动启动原版本，安装包保留供手动重试。

**安装位置随你原来的形态**：

| 原本的形态 | 更新后 |
| --- | --- |
| 免安装版（zip 解压运行） | 带 `/DIR=<当前目录>` 装回原处，**保持免安装** |
| 安装版（setup.exe 装的） | 不带 `/DIR`，更新到原安装目录，不会出现两个安装位置 |

> 程序目录必须对当前用户可写（安装程序把程序装到 `%LOCALAPPDATA%\Programs\ForzaGallerySync`
> 正是为此）；程序目录不可写时会提示改为手动更新。
>
> 安装程序方式不可用时（下载失败、拿不到官方哈希清单等）会自动回退到增量更新或完整包，
> 回退策略可在设置页切换。
...
**这里说的增量/完整包是自动更新的回退路径**：首选方式始终是下载并运行官方安装程序，
只有它不可用时（下载失败、拿不到官方哈希清单等）才回退到这里。

回退时只下载**相对上一版真正变化**的文件，而不是每次都重下约 120 MB 的完整包。
...
本地缺失又无来源），完整包那条路径与之前完全一致。设置页可在"增量更新"与"完整包"
之间切换**回退策略**（安装程序不可用时用哪个）。
...
- **卸载时询问是否删除用户数据**（默认保留）：对话框会列出实际路径
  （`<安装目录>\forza_sync.db` 与 `%APPDATA%\forza-sync\config.json`），并说明
  **照片文件在下载目录里、不受该选项影响**。勾选才删，不勾选就保留，方便重装后免于重新登录。
...
- **安装程序是更新的首选方式**：自动更新会下载并运行官方安装程序。安装失败或被取消时
  会启动原版本、保留安装包供手动重试；程序目录不可写（例如装在 `Program Files`）时只提示手动更新。
- **回退方式（增量/完整包）仍是"覆盖"语义**：它只覆盖包内存在的文件、不删除多余文件，
  因此从旧布局升级到新布局时旧文件可能残留（不影响运行）。安装程序方式不受此限制
  （Inno 会按清单清理它自己安装的文件）。
- 安装/更新日志分别在 `%TEMP%\ForzaGallerySync-update.log` 与其同名的 `.setup` 文件里。
